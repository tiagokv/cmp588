...
void LoadGeomFile(int& node_num, PerfectMatching::REAL*& x_array, GeomPerfectMatching::REAL*& y_array, char* filename)
{
	int i = 0, i_tmp, DIM = 0;
	char LINE[1000];
	...
	while (fgets(LINE, sizeof(LINE)-1, fp))
	{
		if (sscanf(LINE, "DIMENSION : %d", &node_num) == 1)
		{
			...
			x_array = new GeomPerfectMatching::REAL[node_num];
			y_array = new GeomPerfectMatching::REAL[node_num];
			continue;
		}
...
int main(int argc, char* argv[])
{
	...
	{
		int geom_node_num;
		GeomPerfectMatching::REAL* x_array;
		GeomPerfectMatching::REAL* y_array;
